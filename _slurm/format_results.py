...
def write_combined_fdr(rawdf, colname_genes, colname_adjpval, package, method=None, log_pvalue=False):
    ...
    if not log_pvalue:
        ...
        x = df[df["log_adj_pvalue"] < numpy.log10(0.05)]
...
def write_combined_score(rawdf, colname_genes, colname_score, package, method=None):
...
write_combined_fdr(giotto1, colname_genes="genes", colname_adjpval="adj.p.value", package="giotto", method="binSpect_kmeans")

giotto2 = pd.read_table("results/svgenes/giotto/results_binSpect_rank.csv")
write_combined_fdr(giotto2, colname_genes="genes", colname_adjpval="adj.p.value", package="giotto", method="binSpect_rank")
...
write_combined_fdr(gliss, colname_genes="geneID", colname_adjpval="fdr", package="gliss", method=None)
...
write_combined_fdr(raysel, colname_genes="Unnamed: 0", colname_adjpval="q0", package="rayleighselection", method=None)

scgco = pd.read_table("results/svgenes/scgco/results.csv")
write_combined_fdr(scgco, colname_genes="gene", colname_adjpval="fdr", package="scgco", method=None)
...
write_combined_fdr(seurat2, colname_genes="Unnamed: 0", colname_adjpval="adj.pvalue", package="seurat", method="moransi")

sch = pd.read_table("results/svgenes/singlecellhaystack/results.csv")
write_combined_fdr(sch, colname_genes="gene", colname_adjpval="log.p.adj", package="singlecellhaystack", method=None, log_pvalue=True)

somde = pd.read_table("results/svgenes/somde/results.csv")
write_combined_fdr(somde, colname_genes="g", colname_adjpval="qval", package="somde", method=None, log_pvalue=False)

spark1 = pd.read_table("results/svgenes/spark/results_spark.csv")
write_combined_fdr(spark1, colname_genes="Unnamed: 0", colname_adjpval="adjusted_pvalue", package="spark", method="spark")

spark2 = pd.read_table("results/svgenes/spark/results_sparkX.csv")
write_combined_fdr(spark2, colname_genes="Unnamed: 0", colname_adjpval="adjustedPval", package="spark", method="sparkX")

spatialde = pd.read_table("results/svgenes/spatialde/results.csv")
write_combined_fdr(spatialde, colname_genes="g", colname_adjpval="qval", package="spatialde", method=None)
